...
fpathconfPrint(const char *msg, int fd, int name)
{
	...
	if (lim != -1) {        /* Call succeeded, limit determinate */
		...
		if (errno == 0)     /* Call succeeded, limit indeterminate */
			...
			printf("ERROR: fpathconf %s err[%d:%s]\n", msg, errno, strerror(errno));
	}
}
...
/*
output: OS X
gerryyang@mba:syslim$./t_fpathconf 
ERROR: fpathconf _PC_NAME_MAX:  err[22:Invalid argument]
ERROR: fpathconf _PC_PATH_MAX:  err[22:Invalid argument]
_PC_PIPE_BUF:  512
 * */
